...
def show_flow_arrow(bgimg, array_YX, save_path, color = (0,0,255), inter=4):
    bgimg=(((bgimg+1)/2)*255.0).astype(np.uint8)
    b, h, w, c = array_YX.shape
    b, h_img, w_img, c_img = bgimg.shape
    if c_img==1:
        bgimg = np.repeat(bgimg, 3, -1)
    imgs = []
    imgs_b = []
    if b>3:
        b=4
            ...
            for j in range(0, w_img, inter):
                if not (int(j+flow[j/h_img*h, i/w_img*w, 1])>w_img or int(j+flow[j/h_img*h, i/w_img*w, 1])<0 or \
                   int(i+flow[j/h_img*h, i/w_img*w, 0])>h_img or int(i+flow[j/h_img*h, i/w_img*w, 0])<0):
                    img = cv2.arrowedLine(image, (j,i), (int(j+flow[j/h_img*h, i/w_img*w, 1]), int(i+flow[j/h_img*h, i/w_img*w, 0])), color, 1, 8,0,0.2)
                    s=1
...
#    offset_map = tf.transpose(offset_map, [0,1,2, 4, 5, 3])
    ...
    return deform_conv
def conv_2d(x, channels, kernel=3, stride=2, pad=0, pad_type='zero', use_bias=True, sn=False, name='conv_0', use_coor=False):
...
def deconv(x, channels, kernel=4, stride=2, use_bias=True, sn=False, scope='deconv_0', use_coor=False):
